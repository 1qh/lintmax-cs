...
internal static class Rules
{
    ...
    internal static async Task<int> ListAsync()
    {
        ...
        if (!File.Exists(gc))
        {
            await Console.Error.WriteLineAsync("lintmax-cs: globalconfig missing").ConfigureAwait(false);
            return 1;
        }
        ...
        {
            var t = line.Trim();
            if (t.StartsWith("dotnet_diagnostic.", StringComparison.Ordinal)
        ...
        await Console.Error.WriteLineAsync(
            $"{enabled.ToString(System.Globalization.CultureInfo.InvariantCulture)} forced rules + every enabled-by-default rule (-> error)").ConfigureAwait(false);
        return 0;
    }
}
